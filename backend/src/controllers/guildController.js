...
const read = (req, res) => {
  models.guild
    ...
    .catch((err) => {
      console.error(`Error retrieving guild with id ${req.params.id}:`, err);
      res.sendStatus(500);
    });
};
...
const add = (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      res.location(`/guild/${result.insertId}`).sendStatus(201);
    } catch (err) {
...
const edit = (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      const guild = await models.guild.find(guildId);
      if (!guild) {
      ...
      const updatedGuild = {
        ...
        image: req.file ? req.file.path : guild.image,
      };

      const result = await models.guild.update(updatedGuild);
...
const destroy = (req, res) => {
  models.guild
    ...
    .catch((err) => {
      console.error(`Error deleting guild with id ${req.params.id}:`, err);
      res.sendStatus(500);
    });
};
